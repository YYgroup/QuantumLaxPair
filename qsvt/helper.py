...
def random_vector(N, fixed=True, complex=False):
    if fixed:
        np.random.seed(65535)
    dim = 2 ** N
    b = np.random.rand(dim)
    if complex:
        b = b + 1j * np.random.rand(dim)

    return b

def random_matrix(N, fixed=True, complex=False):
    if fixed:
        np.random.seed(65535)

    dim = 2 ** N
    dtype = np.complex128 if complex else np.double
    A = np.zeros((dim, dim), dtype=dtype)
    
    for j in range(dim):
        a_real = np.random.random(dim)
        b_real = np.random.random(dim)
        for i in range(dim):
            if b_real[i] > 0.5:
                a_real[i] *= -1
        if complex:
            a_imag = np.random.random(dim)
            b_imag = np.random.random(dim)
            for i in range(dim):
                if b_imag[i] > 0.5:
                    a_imag[i] *= -1
            row_data = a_real + 1j * a_imag
        else:
            row_data = a_real
