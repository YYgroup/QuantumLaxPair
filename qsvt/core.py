...
def QSVT(
    ...
    rhs = np.asarray(rhs)
    if rhs.size != 0:
        if abs(np.linalg.norm(rhs) - 1) >= EPS:
            raise Exception("The RHS vector should be normalized first! E.g. divide it by \"np.linalg.norm(b)\".")
        qc.initialize(rhs, qubits=range(n))
